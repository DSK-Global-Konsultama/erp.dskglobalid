...
import { CheckCircle, AlertCircle, FileText, Eye, Briefcase, TrendingUp } from 'lucide-react';
      ...
      {projects.length === 0 ? (
        ...
        projects.map(project => {
          ...
          return (
            ...
            >
              {/* Project Info & Work Status */}
              <div className={`mb-4 pb-4 border-b border-gray-200 ${isOverdue ? 'bg-red-50 -mx-6 -mt-6 px-6 pt-6 rounded-t-lg' : ''}`}>
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-600 mb-1">{lead?.company || project.clientName}</div>
                    <div className="text-xs text-gray-500">
                      ID: {project.id}
                      {(handover as any)?.projectPeriod && ` • Period: ${(handover as any).projectPeriod}`}
                    </div>
                  </div>
                  
                  {(handover as any)?.priority && (
                    <div className="flex flex-col items-end gap-2">
                      <span className={`px-2 py-1 rounded border text-xs font-medium ${
                        (handover as any).priority === 'Critical' ? 'bg-red-100 text-red-800 border-red-300' :
                        (handover as any).priority === 'High' ? 'bg-orange-100 text-orange-800 border-orange-300' :
                        (handover as any).priority === 'Medium' ? 'bg-yellow-100 text-yellow-800 border-yellow-300' :
                  ...
                  <div className="flex items-center gap-2">
                    {workStartAllowed ? (
                      ...
                      <>
                        ...
                        <span className="text-sm text-red-600 font-medium">Waiting Payment</span>
                      </>
                    )}
                  </div>
                  <div className="text-sm text-gray-600">
                    Payment Gate: <span className="font-medium">{paymentGateStatus}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 text-sm mb-4">
                <div>
                  ...
                  <div className="text-gray-900">
                    {project.assignedPM || <span className="text-gray-400">Belum Assign</span>}
                  </div>
                </div>
                <div>
