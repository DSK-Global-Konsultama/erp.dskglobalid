...
const isProposalExpired = (proposal: Proposal): boolean => {
  if (proposal.status === 'PROPOSAL_EXPIRED') return true;
  if (!proposal.sentAt || proposal.status !== 'SENT') return false;
...
export function ProposalTab({ leadId, leads, proposals, readOnly = false, onAddProposal, onUpdateProposal, onUpdateLeadStatus }: ProposalTabProps) {
    ...
    <div className="space-y-6">
      ...
      {showProposalDetail && selectedProposal && (
        <ProposalDetailModal
          ...
          onEdit={readOnly ? undefined : (proposal) => {
