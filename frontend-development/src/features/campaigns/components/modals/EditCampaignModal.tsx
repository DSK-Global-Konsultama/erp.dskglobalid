...
import type { Campaign, CampaignStatus, CampaignType, Channel } from '../../../../lib/leadManagementTypes';
...
export function EditCampaignModal({ open, campaign, onClose, onSuccess }: EditCampaignModalProps) {
  const [isAnimatingOut, setIsAnimatingOut] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [noEndDate, setNoEndDate] = useState<boolean>(!(campaign.dateRange?.end || '').trim());
  ...
  const [formData, setFormData] = useState({
    ...
    dateStart: campaign.dateRange?.start || '',
    dateEnd: campaign.dateRange?.end || '',
  ...
  useEffect(() => {
    if (!open) return;
    const end = campaign.dateRange?.end || '';
    setNoEndDate(!end.trim());
    setFormData({
      name: campaign.name,
      type: campaign.type as CampaignType,
      channel: campaign.channel as Channel,
  ...
  const validate = () => {
    ...
    if (!noEndDate && new Date(formData.dateStart) > new Date(formData.dateEnd)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const updated = await campaignsService.update(campaign.id, {
      ...
      toast.error(err?.message || 'Gagal update campaign');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    ...
    >
      <style>{`
        ...
        [role="listbox"] { z-index: 10000 !important; }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setFormData((prev) => ({ ...prev, channel: v as Channel }))}
                  >
                    <SelectTrigger className="w-full">
                ...
                {/* Topic Tag */}
                <div className="md:col-span-2">
                  <label className="block text-sm text-gray-700 mb-2">Topic Tag (optional)</label>
                  <Input
                    type="text"
